...
{block name=body}	
	<div class="main_full">		
		...
		<span class="font_small color_dark">{$SERVERS_FOUND}: {count($data)} | (<a class="color_red" href="{$url}servers/add">{$ADD_SERVER}</a>)</span>
			<table class="default_tbl">
			<tr>
				<th>{$STATUS}</th>
				<th>{$NAME}</th>
				<th>{$GAME}</th>
				<th>Info <span class="font_small">( <a href="{$url}servers">{$REFRESH}</a> )</span></th>			
				<th>{$ACTIONS}</th>
			</tr>
		...
		{if $i.boxid}
			<tr>			
				<td>
				{if $i.started}
				<img src="{$systemplate}images/status/ok.png"/>
				{else}
				<img src="{$systemplate}images/status/ok_off.png"/>
				{/if}
				</td>
