...
{block name=title}{$BOX} :: {$LOGS}{/block}
...
{block name=body}
	<a class="butn" href="{$url}box/show/{$box}">&lsaquo; {$BACKTO}</a>
	{if $data}
	<span class="right"><a class="butn ask" href="{$url}box/purge_logs/{$box}">{$PURGE_LOGS}</a></span>
	{/if}
	<br />
